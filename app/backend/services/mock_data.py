...
def _prepare_records(raw_data: Any, table: Table) -> list[dict[str, Any]]:
    ...
    admin_user_id = getattr(settings, 'admin_user_id', None) or os.environ.get('ADMIN_USER_ID')
    ...
    for entry in records_iterable:
        ...
        # Autofill user_id if table has user_id column and it's missing/empty
        if 'user_id' in column_map and (('user_id' not in filtered) or filtered.get('user_id') in (None, '', 0)):
            if admin_user_id:
                visit_name = getattr(column_map['user_id'].type, "__visit_name__", "").lower()
                try:
                    if any(k in visit_name for k in ("char", "text", "string", "varchar")):
                        filtered['user_id'] = str(admin_user_id)
                    else:
                        filtered['user_id'] = int(admin_user_id)
                except Exception:
                    filtered['user_id'] = str(admin_user_id)
            else:
                logger.warning(f"Skip record for table {table.name} due to missing user_id and no ADMIN_USER_ID available")
                continue
        if filtered:
