...
class SalesLeads(Base):
    ...
    automation_state = Column(String, nullable=False, default="eligible", index=True)
    cooldown_until = Column(DateTime(timezone=True), nullable=True, index=True)
    contact_attempt_count = Column(Integer, nullable=False, default=0)
    rotation_count = Column(Integer, nullable=False, default=0)
    last_assigned_sales_id = Column(Integer, nullable=True, index=True)
    last_recycle_reason = Column(String, nullable=True)
    last_automation_at = Column(DateTime(timezone=True), nullable=True)
