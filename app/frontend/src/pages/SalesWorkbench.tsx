...
export default function SalesWorkbench() {
  ...
  return <div className="sales-v3-page app-page">
    ...
    <Dialog open={!!activeTask} onOpenChange={open => { if (!open) { setActiveTask(null); setSupplementalFollowUp(false); } }}><DialogContent><DialogHeader><DialogTitle>{supplementalFollowUp ? '追加跟进记录' : '记录通话结果'} · {activeTask?.lead.business_name}</DialogTitle></DialogHeader><div className="space-y-4"><div className="rounded-lg border border-blue-100 bg-blue-50 p-3 text-sm text-blue-900"><p className="text-xs font-medium text-blue-700">客户号码</p><div className="mt-2 flex flex-wrap gap-2"><CustomerPhoneDial phone={activeTask?.lead.phone || ''} label="RingCentral 拨号" /><Button type="button" size="sm" onClick={() => void copyPhone()}><Clipboard className="mr-1.5 h-3.5 w-3.5" />复制 {activeTask ? e164PhoneNumber(activeTask.lead.phone) : '-'}</Button></div><p className="mt-2 text-xs text-blue-700">{supplementalFollowUp ? '这是独立追加跟进，不会重复增加今日任务完成数。' : '请先手动完成拨打，再如实选择结果；只有保存结果才计入执行。'}</p><p className="mt-2 text-xs text-blue-700">主按钮直接打开 RingCentral App；右侧箭头可改用网页版或系统电话。拨号入口无法自动证明是否接通，最终以员工保存的结果为准。</p></div><div><Label>通话结果</Label><NativeSelect value={outcome} onChange={value => { setOutcome(value); setNextFollowUpAt(suggestedFollowUpValue(value)); }} options={outcomeOptions} /></div><div><Label>下次跟进时间 {['callback', 'interested', 'appointment'].includes(outcome) && '*'}</Label><Input type="datetime-local" value={nextFollowUpAt} onChange={event => setNextFollowUpAt(event.target.value)} disabled={outcome === 'not_interested' || outcome === 'do_not_contact'} /><p className="mt-1 text-xs text-slate-500">系统会按结果先给出建议时间，您可根据商家约定自行调整。</p></div><div><Label>通话备注</Label><Textarea rows={4} value={notes} onChange={event => setNotes(event.target.value)} placeholder="记录商家反馈、需求、预算或禁联原因" /></div><div className="rounded-lg bg-slate-50 p-3 text-xs text-slate-600">当前为员工手动记录，系统不把 RingCentral“已连接”当作真实拨打或接通证明。有意向、已预约和禁联必须填写完整备注。</div><div className="flex justify-end gap-2"><Button variant="outline" onClick={() => { setActiveTask(null); setSupplementalFollowUp(false); }}>取消</Button><Button disabled={saving} onClick={() => void submitResult()}>{saving ? '保存中...' : supplementalFollowUp ? '记录追加跟进' : '完成并记录'}</Button></div></div></DialogContent></Dialog>
